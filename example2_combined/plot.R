...
plotd <- function(res, data, i=1, j=2, reso=30, cols=rev(gray.colors(100))) {
  ...
  image(a_im, x=xc, y=yc, col=cols, xlab=NA, ylab=NA, xaxt="n", yaxt="n")
  ## add datapoints
  pchs <- c(1, 2, 5)
  pchs_sel <- c(16, 17, 18)
  points(data[,c(i,j)+1], col=data[,1], cex=.6, pch=pchs[data[,1]])
  ## add clusters
  plot_add_contour(res, i, j)
  ## add sampled points
  points(xy_data, col=y, pch=pchs_sel[y], cex=1.1, lwd=1)
}


cross_plot <- function(res, data){
  d <- ncol(data)-1
