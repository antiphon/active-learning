...
plotd <- function(res, data, i=1, j=2, reso=30, zlim=NULL, cols=rev(gray.colors(100))) {
  ...
  if(res$est_alpha) {
    a_hat <- labeller_predict_alpha(res$labeller,xy)
  }else a_hat <- rep(1, nrow(xy))
  a_im <- log(matrix(a_hat, ncol=floor(asp*reso)))
  xname <- colnames(data)[i+1]
  yname <- colnames(data)[j+1]
  if(is.null(zlim)) zlim <- range(a_im)
  image(a_im, x=xc, y=yc, col=cols, xlab=xname, ylab=yname, xaxt="n", yaxt="n", zlim=zlim)
  ## add datapoints
  points(data[,c(i,j)+1], col=data[,1], cex=.6, pch=19)
  ## add clusters
  plot_add_contour(res, i, j)
  ## add sampled points
  points(xy_data, col=1+length(res$classifier$classes), cex=1.2, lwd=1)
}
