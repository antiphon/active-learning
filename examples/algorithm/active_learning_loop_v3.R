...
#' v3: allow for user learning before AL steps.
...
active_learning <- function(data0, # the initial learning sample for the classifier
  ...
  cat3 <- if(debug>1) cat else function(...) NULL
  if(is.null(result)) {
    result <- list( 
    ...
    if(!missing(test_data)) {
      ...
      result$test_scores <- rbind(result$test_scores,  fscore(test_data[,1], pred_test))
    }
  }
  ...
  # Estimate the labeller first
  if(length(train_user_with) > 0) {
    cat2("Pre-learn labeller")
    xnew <- dataUnlabeled[train_user_with,]
    answer <- oracle(xnew)
    hnew <- answer$h
  ...
  for(i in 1:nsteps) {
    ...
    CLA0 <<- result$classifier
    CLASSIFIER <<- result$classifier <- classifier_update(result$classifier, xnew,  hnew)
    if(result$est_alpha){
      cat2("updateLabeller ")
      LABELLER <<- result$labeller <- labeller_update(result$labeller, xnew,  hnew)
    }
    ## training error
